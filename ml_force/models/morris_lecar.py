...
class MorrisLecarBlockNP:
    def __init__(
        ...
        self.x_hat_rec = np.zeros((self._nt, self.x_hat.size), dtype=float)
    ...
    def T(self):
        self.exp = np.exp(-(self.v - self._v_t) / self._k_p)
    ...
    def euler_step(self) -> None:
        dv = self._dt * self.v_dot() + np.random.rand(
            self._N, 1
        ) * self.w_rand * np.sqrt(self._dt)
        self.n += self._dt * self.n_dot()
        self.s += self._dt * self.s_dot()
        self.v += dv

    def _standardize(self, signal: np.ndarray, i: int):
    ...
    def _update_rls_plot(self, i):
        self.rls_line.set_xdata(self.time[:i])
        self.rls_line.set_ydata(self.x_hat_rec[:i, -1])
        self.rls_sup_line.set_xdata(self.time[:i])
        self.rls_sup_line.set_ydata(self.sup[:i, -1])
        self.rls_ax.set_xlim(0, self.time[i])
        # self.ax.relim()
    ...
    def _update_neural_plot(self, voltage_trace, time_index):
        for line_index in range(len(self.neural_lines)):
            signal = self._standardize(voltage_trace[:, line_index], line_index)
            self.neural_lines[line_index].set_xdata(self.time[:time_index])
            self.neural_lines[line_index].set_ydata(signal[:time_index])
        self.neural_ax.set_xlim(0, self.time[time_index])
        # self.ax.relim()
        # self.ax.autoscale(True, True, True)
    ...
    def _update_decoder_plot(self, decoder_trace, time_index):
        for line_index in range(len(self.decoder_lines)):
            ...
            self.decoder_lines[line_index].set_xdata(self.time[:time_index])
            self.decoder_lines[line_index].set_ydata(
                decoder_trace[:time_index, line_index]
            )
        self.decoder_ax.set_xlim(0, self.time[time_index])
        # self.ax.relim()
        # self.ax.autoscale(True, True, True)
    ...
    def render(
        ...
        if save_all == False:
            ...
            decoder_trace = np.zeros(shape=(self.time.size, n_neurons), dtype=float)
        else:
            voltage_trace = np.zeros(shape=(self.time.size, self._N), dtype=float)
            decoder_trace = np.zeros(shape=(self.time.size, self._N), dtype=float)
        ...
        if live_plot:
            ...
            self._neural_plot_init(voltage_trace, rls_step)
            self._decoder_plot_init(decoder_trace, rls_step)
        ...
        for i in tqdm(range(self._nt)):
            try:
                ...
                if live_plot and (i % plt_interval == 1):
                    ...
                    self._update_neural_plot(voltage_trace, i)
                    self._update_decoder_plot(decoder_trace, i)
        ...
        return random_neuron, voltage_trace, decoder_trace
    ...
    def rls(self, i):
        ...
        self.dec -= (self.Pinv @ self.s) @ error.T  # / (1 + self.s.T @ q)
